...
class Database:
    ...
    @staticmethod
    def InsertDatabase(db, value):
        c = db.cursor()
        c.execute("INSERT INTO tasks (Task, Date) VALUES (?, ?)", value)
        db.commit()
    ...
    @staticmethod
    def UpadteDatabase(db, value):
        c = db.cursor()
        c.execulte("UPDATE tasks SET Task=? WHERE Task=?", value)
        db.commit()
...
def main(page: ft.Page):
    ft.Page.horizontal_alignment = 'center'
    ft.Page.vertical_alignment = 'center'
    ...
    # Adiciona umcountener na pagina
    page.add(
        Container(
            width=1500,
            height=800,
